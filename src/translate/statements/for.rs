...
use crate::{errors::Error, project::Project, sway, translate::{assignment::translate_assignment_expression, translate_expression, TranslatedDefinition, TranslationScope}};
...
pub fn translate_for_statement(
    ...
    if let Some(initialization) = initialization.as_ref() {
        ...
        let mut statement = translate_statement(project, translated_definition, &scope, initialization.as_ref())?;
    ...
    let condition = if let Some(condition) = condition.as_ref() {
        translate_expression(project, translated_definition, &scope, condition.as_ref())?
    } else {
    ...
    let mut body = match body.as_ref() {
        ...
        Some(body) => match translate_statement(project, translated_definition, &scope, body.as_ref())? {
    ...
    if let Some(update) = update.as_ref() {
        body.statements.push(sway::Statement::from(
            match update.as_ref() {
                ...
                _ => translate_expression(project, translated_definition, &scope, update.as_ref())?
            }
        ));
    }
