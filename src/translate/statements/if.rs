...
pub fn translate_if_statement(
    ...
    let condition = translate_expression(project, module.clone(), scope, condition)?;

    let then_body = match translate_statement(project, module.clone(), scope, then_body)? {
    ...
    let else_if = if let Some(else_if) = else_if.as_ref() {
        match translate_statement(project, module.clone(), scope, else_if.as_ref())? {
