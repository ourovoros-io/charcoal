...
pub fn translate_literal_expression(
    ...
    match expression {
        ...
        solidity::Expression::NumberLiteral(_, value, _, _) => Ok(sway::Expression::from(
            sway::Literal::DecInt(value.parse().unwrap(), None),
        )),

        solidity::Expression::RationalNumberLiteral(_, _, _, _, _) => Ok(
            sway::Expression::create_todo(Some(format!("rational number: {expression}"))),
        ),

        solidity::Expression::HexNumberLiteral(_, value, _)
        | solidity::Expression::AddressLiteral(_, value) => {
