...
pub fn translate_binary_expression(
    ...
    let abi_check = |lhs_type: &sway::TypeName, rhs: &mut sway::Expression, rhs_type: &mut sway::TypeName| -> bool {
        ...
        match &type_name {
            sway::TypeName::Identifier { name, generic_parameters } => match (name.as_str(), generic_parameters.as_ref()) {
                ...
                ("u8" | "u16" | "u32" | "u64" | "u256", None) => {
                    ...
                    module.borrow_mut().ensure_use_declared(format!("sway_libs::signed_integers::i{bits}::*").as_str());
