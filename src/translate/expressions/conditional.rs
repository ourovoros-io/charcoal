...
pub fn translate_conditional_operator_expression(
    ...
    Ok(sway::Expression::from(sway::If {
        condition: Some(translate_expression(project, module.clone(), scope, condition)?),
        then_body: sway::Block {
            statements: vec![],
            final_expr: Some(
                translate_expression(project, module.clone(), scope, then_value)?
            ),
        },
        else_if: Some(Box::new(sway::If {
            ...
            then_body: sway::Block {
                statements: vec![],
                final_expr: Some(
                    translate_expression(project, module.clone(), scope, else_value)?
                ),
            },
            else_if: None,
        })),
