        ...
        if let sway::Expression::MemberAccess(member_access) = expression {
                ...
                != sway::Expression::create_identifier(format!("storage::{namespace_name}"))
            {
                ...
                if let Some(struct_definition) = module.borrow().structs.iter().find(|s| {
                    ...
                    if s.name != *name
                        || s.generic_parameters.is_some() != generic_parameters.is_some()
                    {
                        return false;
                    }
                    ...
                    {
                        if lhs_generic_parameters.entries.len()
                            != rhs_generic_parameters.entries.len()
                        {
                            return false;
                        }
                    }
                    ...
                    let struct_definition =
                        struct_definition.implementation.as_ref().unwrap().borrow();
                            ...
                            (
                                    ...
                                    if generic_parameters.entries.len() == 1 =>
                                {
                                    match &generic_parameters.entries[0].type_name {
                                        sway::TypeName::Identifier {
                                            ...
                                            match (name.as_str(), generic_parameters.as_ref()) {
                                                ("StorageString", None) => {
                                                    ...
                                                    module
                                                        .borrow_mut()
                                                        .ensure_use_declared("std::string::*");
                                        ...
                                        _ => {
                                            coerce_expression(rhs, rhs_type_name, &storage_key_type)
                                                .unwrap()
                                        }
                                    }
                                }
