...
pub fn translate_function_call_expression(
    ...
    match function {
        solidity::Expression::Type(_, ty) => {
            ...
            match ty {
                solidity::Type::Address => translate_address_type_casting(
                    project,
                    translated_definition,
                    scope,
                    expression,
                ...
                solidity::Type::Payable => translate_payable_type_casting(
                    project,
                    translated_definition,
                    scope,
                    expression,
                ...
                solidity::Type::Int(bits) => translate_int_types_casting(
                    project,
                    translated_definition,
                    scope,
                    expression,
                ...
                solidity::Type::Uint(bits) => translate_uint_types_casting(
                    project,
                    translated_definition,
                    scope,
                    expression,
                ...
                solidity::Type::Bytes(byte_count) => translate_byte_types_casting(
                    project,
                    translated_definition,
                    scope,
                    &arguments[0],
                ...
                solidity::Type::DynamicBytes => translate_dynamic_bytes_type_casting(
                    project,
                    translated_definition,
                    scope,
                    &arguments[0],
                ...
                solidity::Type::String => translate_string_type_casting(
                    project,
                    translated_definition,
                    scope,
                    expression,
            ...
            match container.as_ref() {
                    ...
                    .as_str()
                {
                    ...
                    name => {
                        ...
                        return Err(Error::Wrapped("Failed to translate".into()));
                    }
                },
                ...
                _ => {
                    ...
                    let converter = |argument: &sway::Expression,
                                     argument_type_name: &sway::TypeName|
                     -> sway::Expression {
                        match argument_type_name {
                            sway::TypeName::Identifier {
                                ...
                                ("u256", None) => sway::Expression::create_function_calls(
                                    None,
                                    &[
...
/// shound be removed at some point
fn translate_builtins(
    project: &mut Project,
    translated_definition: &mut TranslatedDefinition,
    scope: Rc<RefCell<TranslationScope>>,
    function: &solidity::Expression,
...
fn translate_address_type_casting(
    project: &mut Project,
    translated_definition: &mut TranslatedDefinition,
    scope: Rc<RefCell<TranslationScope>>,
    expression: &solidity::Expression,
            ...
            sway::TypeName::Array { type_name: element_type_name, length } => match element_type_name.as_ref() {
                sway::TypeName::Identifier { name, generic_parameters } => match (name.as_str(), generic_parameters.as_ref()) {
                    ...
                    _ => panic!(
                        ...
                        match project.loc_to_line_and_column(&translated_definition.path, &argument.loc()) {
                            Some((line, col)) => format!("{}:{}:{} - ", translated_definition.path.to_string_lossy(), line, col),
                            None => format!("{} - ", translated_definition.path.to_string_lossy()),
                        },
                    ),
...
// payable(x) => x
fn translate_payable_type_casting(
    project: &mut Project,
    translated_definition: &mut TranslatedDefinition,
    scope: Rc<RefCell<TranslationScope>>,
    expression: &solidity::Expression,
...
fn translate_int_types_casting(
    project: &mut Project,
    translated_definition: &mut TranslatedDefinition,
    scope: Rc<RefCell<TranslationScope>>,
    expression: &solidity::Expression,
...
fn translate_uint_types_casting(
    project: &mut Project,
    translated_definition: &mut TranslatedDefinition,
    scope: Rc<RefCell<TranslationScope>>,
    expression: &solidity::Expression,
...
/// shound be removed at some point
// bytesN(x) => ???
fn translate_byte_types_casting(
    project: &mut Project,
    translated_definition: &mut TranslatedDefinition,
    scope: Rc<RefCell<TranslationScope>>,
    argument: &solidity::Expression,
...
// bytes(x) => ???
fn translate_dynamic_bytes_type_casting(
    project: &mut Project,
    translated_definition: &mut TranslatedDefinition,
    scope: Rc<RefCell<TranslationScope>>,
    argument: &solidity::Expression,
...
// string(x) => ???
fn translate_string_type_casting(
    project: &mut Project,
    translated_definition: &mut TranslatedDefinition,
    scope: Rc<RefCell<TranslationScope>>,
    expression: &solidity::Expression,
