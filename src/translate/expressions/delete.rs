...
pub fn translate_delete_expression(
    ...
    let (variable, expr) = translate_variable_access_expression(project, module.clone(), scope, expression)?;
        ...
        if let Some(storage_type_name) = type_name.storage_key_type() {
            if let sway::TypeName::Identifier { name, generic_parameters } = storage_type_name {
                if let ("StorageMap" | "StorageVec", Some(_)) = (name.as_str(), generic_parameters.as_ref()) {
                    ...
                    let index = translate_expression(project, module.clone(), scope, index.as_ref().unwrap())?;
    ...
    create_assignment_expression(project, module.clone(), scope, "=", &expr, &variable, &value, &type_name)
}
